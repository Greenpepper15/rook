...
- Object: Added `isolatedRootPool` to `CephObjectStore` (with `sharedPools`) and `CephObjectRealm` (multisite) to store a new store's or realm's RGW topology records (realm/zonegroup/zone/period) in a RADOS namespace of the `.rgw.root` pool named after the realm, instead of sharing the un-namespaced `.rgw.root` across all object stores. See [Isolating the RGW topology pool](Documentation/Storage-Configuration/Object-Storage-RGW/object-storage.md#isolating-the-rgw-topology-pool). Do not downgrade the operator below this release while such stores or realms exist: gateways keep resolving the isolated topology via a mon config database backstop, but an older operator will create a conflicting realm in the shared `.rgw.root` that must be removed after upgrading back.
