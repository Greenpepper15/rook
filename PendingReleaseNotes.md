...
- RGW least-privilege CephX caps: `CephObjectStore` gateways can run with OSD capabilities scoped to exactly the RADOS pools and namespaces their zone references, instead of cluster-wide `osd allow rwx`. Opt-in and disabled by default via `spec.gateway.cephxLeastPrivilege`; existing object stores are unaffected. See the [CephObjectStore Gateway Settings](Documentation/CRDs/Object-Storage/ceph-object-store-crd.md#cephx-least-privilege) for details.
